...
class PDFComparerApp:
    ...
    def create_widgets(self):
        ...
        compare_button = tk.Button(compare_frame, text="Rationalise (Excel)", font=("Helvetica", 10, "bold"),
                                   command=lambda: threading.Thread(target=self.compare_pdfs).start(), width=20,
                                   height=2, bg="white")
        compare_button.pack(side=tk.LEFT)

        # Button for CSV Report Generation
        csv_button = tk.Button(compare_frame, text="Rationalise (CSV)", font=("Helvetica", 10, "bold"),
                               command=lambda: threading.Thread(target=self.compare_pdfs_csv).start(), width=20,
                               height=2, bg="white")
        csv_button.pack(side=tk.LEFT, padx=(15, 0))

        # Button for Percentage Match operation
        similarity_button = tk.Button(compare_frame, text="Percentage Match (Excel)", font=("Helvetica", 10, "bold"),
        ...
        # Button for CSV Similarity Report Generation
        similarity_csv_button = tk.Button(compare_frame, text="Percentage Match (CSV)", font=("Helvetica", 10, "bold"),
                                          command=lambda: threading.Thread(target=self.compare_similarity_csv).start(),
                                          width=20, height=2, bg="white")
        similarity_csv_button.pack(side=tk.LEFT, padx=(15, 0))
    ...
    def compare_pdfs_csv(self):
        ...
        current_time = datetime.now()
        format_time = current_time.strftime("%Y%m%d%H%M%S")
        output_csv_common = os.path.join(output_folder, f"rationalized_result_common_{format_time}.csv")
        output_csv_matrix = os.path.join(output_folder, f"rationalized_result_matrix_{format_time}.csv")
    ...
    def compare_similarity_csv(self):
        ...
        current_time = datetime.now()
        format_time = current_time.strftime("%Y%m%d%H%M%S")
        output_csv_paragraphs = os.path.join(output_folder, f"percentage_report_paragraphs_{format_time}.csv")
        output_csv_matrix = os.path.join(output_folder, f"percentage_report_matrix_{format_time}.csv")

        # Write Paragraphs to CSV
        with open(output_csv_paragraphs, mode='w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)

...
def write_excel_results(output_folder, filename_prefix, common_hashes, matrix):
...
def write_csv_results(output_folder, filename_prefix, common_hashes, matrix):
    current_time = datetime.now()
    format_time = current_time.strftime("%Y%m%d%H%M%S")
...
def write_html_results(output_folder, filename_prefix, common_hashes, matrix):
    current_time = datetime.now()
    format_time = current_time.strftime("%Y%m%d%H%M%S")
...
class PDFComparerApp:
    ...
    def create_heading_frame(self):
        ...
        heading_label = self.create_label(heading_frame, "Content Rationalizer", font=("Helvetica", 26, "bold"), bg="#1a1a2e", fg="white")
    ...
    def create_folder_frame(self, label_text, path_variable, browse_command):
        ...
        frame.pack(pady=20)
        self.create_label(frame, label_text).pack(side=tk.LEFT)
        self.create_entry(frame, path_variable, width=50).pack(side=tk.LEFT)
        self.create_button(frame, "Browse", browse_command).pack(side=tk.LEFT, padx=(10, 0))
    ...
    def create_compare_buttons(self):
        ...
        self.create_compare_button(compare_frame, "Rationalise (HTML)", self.compare_pdfs_html, padx=(15, 0))
        self.create_compare_button(compare_frame, "Percentage Match (Excel)", self.compare_similarity_excel, padx=(15, 0))
    ...
    def create_compare_button(self, frame, text, command, padx=None):
        button = self.create_button(frame, text, lambda: threading.Thread(target=command).start(), font=("Helvetica", 10, "bold"), width=20, height=2, bg="white")
    ...
    def compare_pdfs(self):
        ...
        try:
            ...
            write_excel_results(output_folder, "rationalized_result", common_hashes, matrix)
        except Exception as e:
    ...
    def compare_pdfs_csv(self):
        ...
        try:
            all_paragraphs = process_pdfs_in_parallel(pdf_paths)
            common_hashes, matrix = generate_common_hashes_and_matrix(pdf_paths, all_paragraphs)
            write_csv_results(output_folder, "rationalized_result", common_hashes, matrix)
        except Exception as e:
            logging.error(f"Error during comparison: {str(e)}")
    ...
    def compare_pdfs_html(self):
        ...
        try:
            all_paragraphs = process_pdfs_in_parallel(pdf_paths)
            common_hashes, matrix = generate_common_hashes_and_matrix(pdf_paths, all_paragraphs)
            write_html_results(output_folder, "rationalized_result", common_hashes, matrix)
        except Exception as e:
            logging.error(f"Error during comparison: {str(e)}")
